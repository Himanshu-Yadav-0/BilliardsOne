...
const PricingTab = ({ cafes }) => {
    ...
    const handleSavePricing = async (e) => {
        ...
        setSuccess('');
        try {
            const formatPrice = (price) => (price === '' ? null : Number(price));
            ...
            await Promise.all([poolRequest, snookerRequest]);
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="bg-gray-800 rounded-lg p-4 shadow-md">
          ...
          <ul className="space-y-1 text-sm text-gray-300">
            <li>
              <span className="font-medium text-white">Hourly:</span>{" "}
              ₹{poolPricing.hourPrice !== '' ? poolPricing.hourPrice : 'N/A'}
            </li>
            <li>
              <span className="font-medium text-white">Half-Hourly:</span>{" "}
              ₹{poolPricing.halfHourPrice !== '' ? poolPricing.halfHourPrice : 'N/A'}
            </li>
            <li>
              <span className="font-medium text-white">Extra Player:</span>{" "}
              ₹{poolPricing.extraPlayerPrice !== '' ? poolPricing.extraPlayerPrice : 'N/A'}
            </li>
          </ul>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-4 shadow-md">
          ...
          <ul className="space-y-1 text-sm text-gray-300">
            <li>
              <span className="font-medium text-white">Hourly:</span>{" "}
              ₹{snookerPricing.hourPrice !== '' ? snookerPricing.hourPrice : 'N/A'}
            </li>
            <li>
              <span className="font-medium text-white">Half-Hourly:</span>{" "}
              ₹{snookerPricing.halfHourPrice !== '' ? snookerPricing.halfHourPrice : 'N/A'}
            </li>
            <li>
              <span className="font-medium text-white">Extra Player:</span>{" "}
              ₹{snookerPricing.extraPlayerPrice !== '' ? snookerPricing.extraPlayerPrice : 'N/A'}
            </li>
          </ul>
        </div>
      </div>
